...
class TrainDeghostingDataset(Dataset):
    def __init__(self, lq_folder, tg_folder, org_folder, lq_size=128, tg_size=1024, num=None):
        ...
        self.org_paths = [path for path in list_pictures(self.org_folder)]
        self.lq_paths.sort()
        self.tg_paths.sort()
        self.org_paths.sort()
        if num is not None:
            ...
            self.org_paths = self.org_paths[:num]
    ...
    def __getitem__(self, index):
        ...
        org_img = cv2.resize(org_img, (self.tg_size, self.tg_size), interpolation=cv2.INTER_AREA)

        lq = self.transform_tensor(lq_img)
        tg = self.transform_tensor(tg_img)
        org = self.transform_tensor(org_img)
        filename = os.path.basename(lq_path)
        return {"lq": lq, "tg": tg, "org": org, "filename": filename}
