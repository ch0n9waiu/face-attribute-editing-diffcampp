...
class ResUNet(nn.Module):
    ...
    def __init__(self, down_block=DownBlock, up_block=UpBlock, flat_block=FlatBlock, in_nc=3, out_nc=3, max_nc=256,
                 ngf=64, flat_layers=(2, 2, 2, 2), norm_layer=nn.BatchNorm2d, act_layer=nn.ReLU(inplace=True),
                 use_dropout=False):
...
def test():
    ...
    # w = 1024
    # in_channel = 6
    # x = torch.randn([3, in_channel, w, w]).cuda()
    # net = ResUNet(in_nc=in_channel, out_nc=3, ngf=32, max_nc=512).cuda()
    # print(net)
    # preds = net(x)
    # print(preds.shape)
    ...
    import torchsummary
    net = ResUNet(in_nc=6, out_nc=3, ngf=32, max_nc=512).cuda()
    print(net)
    torchsummary.summary(net, (6, 1024, 1024))
