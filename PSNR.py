...
def calculate_psnr_between_images(img1, img2):
    """计算两张图片之间的PSNR"""
...
def process_folders(folder1, folder2, device='cuda', relative_path=None):
    ...
    for filename in tqdm(common_files, desc=f"计算 {relative_path or os.path.basename(folder2)} 的PSNR"):
        ...
        try:
            ...
            psnr = calculate_psnr_between_images(img1, img2)
...
def save_summary_to_txt(output_path, all_results, main_folder1, main_folder2):
    ...
    with open(output_path, 'w') as f:
        ...
        for result in all_results:
            ...
            f.write(f"平均PSNR: {result['mean_psnr']:.2f} dB\n")  # 添加dB单位
            f.write(f"PSNR标准差: {result['std_psnr']:.2f} dB\n")
            f.write("-" * 50 + "\n\n")
...
if __name__ == "__main__":
    ...
    # 输出文件路径
    summary_file = os.path.join(main_folder2, "psnr_summary.txt")
